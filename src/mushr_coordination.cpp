...
MushrCoordination::MushrCoordination(ros::NodeHandle &nh) {
  ...
  if(!nh.getParam("scale", m_sacle)) {m_scale = 1.0;}
  if(!nh.getParam("num_agent", m_num_agent)) {m_num_agent = 8;}
  ...
  m_pub_plan = nh.advertise<mushr_coordination::PlanPoseArray>("/mushr_coordination/plan", 10);
}
...
struct MushrCoordination::Private {
  void solve() {
    ...
    // TODO: publish plan
    mushr_coordination::PlanPoseArray pub;
